...
% Topic #14: Simulation modeling using Kalman filter
% Authors: V.Bobronnikov & M.Trifonov 
% Email: trifonov.m@yahoo.com
% Date(dd-mm-yyyy): 07-03-2021
%==========================================================================
clc; clear; close all
% 1. Initial data
...
D_eta=900; Sig_eta=sqrt(D_eta); % noise variance and standard deviation - Sigma
...
for i=1:N % cycle in steps
    t(i) = i*dt; % time
    X = Fi*X; % Actual current state
    X_data(i,1) = X(1);
    X_data(i,2) = X(2);
    y  =C*X; % ideal measurement 
    eta = Sig_eta*randn(2,1);% measurement errors simulation
    y_izm = y + eta; % actual measurement
    Y_data(i,1) = y_izm(1);
    Y_data(i,2) = y_izm(2);
    if i==1
        Papr = Fi*Kx0*Fi';
...
plot(X_data(:,1),X_data(:,2),'DR'); % actual current position
plot(Y_data(:,1),Y_data(:,2),'xB'); % Measurement points
plot(Xapr_data(:,1),Xapr_data(:,2),'r*');
plot(Xaps_data(:,1),Xaps_data(:,2),'SK');
